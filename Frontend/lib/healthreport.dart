...
class _HealthProfilePageState extends State<HealthProfilePage> {
  ...
  Future<void> _loadProfileData() async {
    try {
      ...
      // Fetch health score & risk level + questionnaire dynamically
      ...
      if (qSnap.docs.isNotEmpty) {
        ...
        for (var key in data.keys) {
          if (!['phone', 'healthScore', 'riskLevel', 'riskColor', 'submittedAt']
              .contains(key)) {
            if (data[key] is List) {
  ...
  /// ✅ Store in state variable, not local variable
  ...
  Future<void> _downloadReport() async {
    // ✅ Keep the same functionality for now
        ...
        Row(
          children: [
            CircularPercentIndicator(
              ...
              lineWidth: 10,
              percent:
                  (userAqi != null && userAqi! <= 500) ? (userAqi! / 500.0) : 0,
  ...
  child: Column(
    ...
    children: [
      ...
      if (sensorId != null) // only show if sensorId exists
        Text(
          ...
          Row(
            children: [
              CircularPercentIndicator(
                ...
                center:
                    Text("$healthScore", style: const TextStyle(color: Colors.white)),
                progressColor: Colors.white,
                backgroundColor: Colors.white24,
              ),
              const SizedBox(width: 16),
