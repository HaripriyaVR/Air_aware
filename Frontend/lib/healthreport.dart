...
class HealthScorePage extends StatelessWidget {
  ...
  const HealthScorePage({Key? key, required this.phone}) : super(key: key);

  // ✅ Helper function to parse #hex color to Flutter Color
  ...
  try {
    ...
    // If parsing fails, return a safe default color
    debugPrint('Invalid color format: $hex, defaulting to grey. Error: $e');
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (context, snapshot) {
          ...
          final score = data['healthScore'] ?? 0;
          final level = data['riskLevel'] ?? 'Unknown';
          final colorHex = data['riskColor'] ?? '#CCCCCC';
          final color = _hexToColor(colorHex); // ✅ Use safe hex parser
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 40),
                Text(
                  'This score is based on your symptoms, lifestyle, and environment.',
...
import 'dart:html' as html; // for web download — remove for non-web targets
...
class _ProfilePageState extends State<HealthProfilePage> {
  ...
  Future<void> fetchAQI(double lat, double lon) async {
    ...
    final url = Uri.parse('http://localhost:5000/user-aqi?lat=$lat&lon=$lon');
  ...
  // --- helper methods you asked to keep / use ---
  ...
  // your existing combined risk functions (kept unchanged)
