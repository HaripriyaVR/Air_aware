...
class _AQIDashboardPageState extends State<AQIDashboardPage> {
  static const String _baseUrl = 'http://192.168.43.104:5000';  // Updated for Android emulator
  static const String _endpoint = '/realtime';
  ...
  _sensorLocations.forEach((id, sensorPos) {
    ...
    debugPrint('[DISTANCE] Sensor: ${SensorNameMapper.displayName(id)} ($id), '
  ...
  if (minDist > 2.0) {
    debugPrint('[INFO] No sensors found within 2 km.');
    return null;
  }
  return closest;
}
  ...
  try {
    ...
    // Attach sensor info
    sensorData['sensorId'] = sensorId;
    sensorData['sensorName'] = SensorNameMapper.displayName(sensorId); // ✅ Here it applies
