...
import 'dart:convert';
import 'questions.dart'; // Import your questionnaire page
...
class _OtpVerificationScreenState extends State<OtpVerificationScreen> {
  String otpCode = "";
  String backendUrl = 'http://localhost:5000'; // Replace with actual IP on physical device
  ...
  Future<void> verifyOtp() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && result['status'] == 'approved') {
        ...
        // 🔹 Check Firestore for first-time login
        // 🔹 Check Firestore for first-time login
...
if (qSnap.docs.isNotEmpty) {
  // Already filled questionnaire → go to Profile Page
  Navigator.pushAndRemoveUntil(
    context,
    MaterialPageRoute(builder: (_) => ProfilePage(phone: widget.phoneNumber)),
    (route) => false,
  );
} else {
  // First time → go to Questionnaire Page
  Navigator.pushAndRemoveUntil(
    context,
    MaterialPageRoute(
      builder: (_) => QuestionnairePage(phone: widget.phoneNumber),
    ),
    (route) => false,
  );
}
        ...
        showSnackbar("❌ Verification failed: ${result['error'] ?? 'Invalid code'}");
      }
    } catch (e) {
